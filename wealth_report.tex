...
\input{results/stock_dd_macros.tex}   % computed figures for the Market-Concentration section
...
\emph{soft} rebalancing (steer only new contributions toward the under-weight legs, never sell), and
...
the simulation. The spread in median wealth runs about \textbf{4$\times$} (roughly \$2.1M to \$8.7M),
and on a paired test -- comparing strategies on the \emph{same} simulated histories -- the major
differences are distinguishable from zero. So which strategy an investor picks materially changes
where they land. Not every pair is distinguishable: some adjacent mixes (for example 40/30/30 versus
30/30/40 on the Sharpe ratio) test as statistically indistinguishable. But the large differences that
drive the conclusions are real, not noise.
...
only strategies sitting exactly at the risk threshold move across it. The conclusions below are drawn
...
Over the 46 years, the same contribution stream produced median final balances of about \textbf{\$2.1M
to \$8.7M} depending on the strategy and account -- a roughly 4$\times$ range. The large differences
between strategies exceed the noise of the simulation, so the choice of strategy and account changes
the outcome.

More equity produced more wealth and deeper drawdowns together: the highest-wealth strategies (the
equity-heavy mixes, \$6--8M) also carried the deepest terminal drawdowns ($-40\%$ to $-59\%$). No
...
per unit of terminal pain, the efficient set is small -- five strategies in the taxable account, four in
the tax-free account (Section~\ref{sec:frontier}). Three appear in both: the
\textbf{90/10$\to$30/70 target-date glide (hard-rebalanced)}, \textbf{All Weather}, and the
...
conditions turn hard long predates any window in our data. Several legs (international stocks, technology,
...
the account.} The same contribution stream over 46 years produced anywhere from about \textbf{\$2.1M
to \$8.7M} depending on the choices -- roughly a \textbf{4$\times$} range. The report makes no claim
about whether any of those endpoints is adequate for a retirement; it measures the gap between a poor
set of choices and a good one, which is millions of dollars.
...
strategies (80/20, aggressive; \$6--8M) also had the deepest terminal drawdowns ($-40\%$ to $-59\%$);
...
\item \textbf{Only a handful of strategies were efficient.} Of the 56, five in the taxable account and
...
\paragraph{Taxable.} Five strategies are efficient within the $-30\%$ threshold:
...
\midrule
\input{results/eff_taxable.tex}\\
\bottomrule
\end{tabular}
\end{center}
\noindent(The 50/25/25 mix is efficient at a looser threshold but its terminal pain of $-31\%$ falls
just outside $-30\%$.)
...
Strategy & Median wealth & Terminal pain & W/TP \\
\midrule
\input{results/eff_taxfree.tex}\\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Reading the frontier.} The two efficient sets share three names -- the
\textbf{TDF 90/10$\to$30/70 glide (hard)}, \textbf{All Weather}, and the \textbf{Permanent Portfolio
(hard)} -- with 60/40 (hard) joining in the tax-free account and a stocks/gold/bonds glide in the
taxable one. Within each set the choice is by preference: the Permanent Portfolio is the calmest
(terminal pain near $-16$ to $-18\%$), the richer members carry more terminal pain. Both efficient
sets are unchanged under a second random seed.
...
30/30/40 flat on both axes in both accounts: in the taxable account \$3.22M against \$3.10M with a
shallower terminal pain ($-19\%$ against $-25\%$), and by more in the tax-free account. So ``risky
...
$-33$ to $-34\%$ terminal pain -- past the $-30\%$ threshold, and deeper than holding flat -- which
...
drawdown reaches $-36.5\%$, against $-28.8\%$ for holding flat. But that loss lands when the balance is
...
REITs, and $-46\%$ for long Treasuries. For the intermediate-bond leg the window is a four-decade bull
...
every horizon (Appendix~\ref{sec:horizon}), and carries terminal pain of $-18$ to $-19\%$. It holds
...
\section{Stocks, and an Argument for Diversification}\label{sec:concentration}

A natural objection to a diversified, gold- and bond-heavy portfolio is that one could simply own the
handful of great technology companies that have led the market. The record of individual stocks is a
...
\midrule
\input{results/stock_dd_rows.tex}\\
\bottomrule
\end{tabular}
\end{center}
}
...
500, against roughly \TopTenThen\% at the 2000 peak. A repeat of the dot-com decline --- the Nasdaq-100,
...
Finally, the belief that ``the market always comes back'' describes US history; it is not a law that
holds everywhere. Japan's Nikkei~225 peaked at the end of \JapanPeakYear, fell \JapanDecline\% to its
\JapanTroughYear\ low, and did not close back above its \JapanPeakYear\ high until \JapanRecoverYear{}
--- \JapanRecoverYears\ years later. An investor who bought the leading index of what was then the world's
second-largest economy, at its peak, waited more than three decades just to break even on price. A
concentrated position that has to recover is betting that the US market will keep being the exception.

This is why the argument of this report is for diversification \emph{across} assets that do not move
together, not only \emph{within} one of them. Ray Dalio calls a portfolio of fifteen or twenty
uncorrelated return streams the ``Holy Grail of investing,'' because combining them can cut risk sharply
without giving up expected return.\footnote{Ray Dalio, \emph{Principles} (Simon \& Schuster, 2017).} A
broad stock index diversifies away the risk that any one company fails; it does nothing about the risk
that stocks as a class have a decade --- or, as in Japan, a generation --- that does not come back. That
is the gap the gold and bond holdings are there to cover.

\smallskip
\noindent\footnotesize Market-concentration figures are as of \ConcAsOf{} and, unlike the drawdowns
above, are not computed from price data: \ConcSources{} \JapanSource\normalsize

\section{Questions a Careful Reader Will Ask}\label{sec:critiques}

This section anticipates the critiques of the analysis itself.
...
Nominal --- and that is the right unit, not a shortcoming. The investor reads their account balance in
...
\paragraph{Several legs use proxy or constructed data before the mid-1990s.} They do. International
...
interpolated and does not carry day-to-day volatility -- the one leg whose pre-fund daily path is
...
\paragraph{Constructed Treasury and REIT series.} Three legs have no investable fund reaching back to
...
is the one leg whose pre-fund daily path is synthetic at the daily frequency.
\end{itemize}
...
\paragraph{Costs.} The gold leg carries a \textbf{0.4\% annual management fee} (a physical-bullion ETF
...
\noindent Annual total returns for each leg (stocks and bonds include dividends; gold is price) and for
...
year-to-date through June~1.) The blend is a weighted combination of the legs, so its return in any
year is mechanically the sum of the legs' returns at their weights -- which is what lets the other legs
cushion a fall in one.
...
\midrule
\input{results/calendar_rows_left.tex}\\
\bottomrule
\end{tabular}
\end{minipage}\hfill
\begin{minipage}[t]{0.46\textwidth}
...
Year & Stocks & Gold & Bonds & 30/30/40 \\
\midrule
\input{results/calendar_rows_right.tex}\\
\bottomrule
\end{tabular}
\end{minipage}
\end{center}
...
\midrule
\input{results/downyears_rows.tex}\\
\bottomrule
\end{tabular}
\end{center}
}
...
\midrule
\input{results/asset_dd_rows.tex}\\
\bottomrule
\end{tabular}
\end{center}
}
...
rate spike. The 1980--2026 record is, for the intermediate-bond leg, essentially one long bull market
...
\midrule
\input{results/horizon_rows.tex}\\
\bottomrule
\end{tabular}
\end{center}
}
...
\midrule
\input{results/horizon_div_rows.tex}\\
\bottomrule
\end{tabular}
\end{center}
}

...
the simulation. On a paired test -- comparing strategies on the \emph{same} simulated histories -- the
major differences are distinguishable from zero. So which strategy an investor picks materially changes
...
Over the 46 years, the same contribution stream produced median final balances of about \textbf{\$2.0M
to \$8.7M} depending on the strategy and account. The large differences between strategies exceed the
...
the account.} The same contribution stream over 46 years produced anywhere from about \textbf{\$2.0M
to \$8.7M}.
...
choice is by preference: the Permanent Portfolio is the calmest (terminal pain near $-16$ to $-18\%$),
...
40\% of its wealth, so it no longer dominates: \textbf{60/40 (hard)} now sits below the frontier, and
...
30/30/40 flat on both axes in both accounts: in the taxable account \$3.03M against \$3.01M with a
shallower terminal pain ($-19\%$ against $-26\%$), and by more in the tax-free account (\$4.93M
against \$3.73M, $-18\%$ against $-25\%$). So ``risky
